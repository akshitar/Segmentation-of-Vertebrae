...
h = fspecial('unsharp'); % filtering for sharpness
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% STEP 1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
n = 3000;
for imageNumber = 1 : 40
filename = strcat('negativeImages/',allImages(imageNumber).name);
...
for i=1:length(dummy)
    ...
    extra = zeros([length(winMatrix),2]);
    ...
    auxiliaryPoints = zeros(length(winMatrix),1);
...
train_pos_clusters = agglomerativeCluster( patchMatrix, 7);
show_clusters( train_pos_clusters, patchMatrix, windowSize );

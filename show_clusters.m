...
% patches - patch data
function show_clusters( clusters, patches, windowSize )
...
for i=0:clusters_at_once:num_clusters
  ...
  for j=1:clusters_at_once
    if i+j <= num_clusters
      ...
      imshow( uint8(reshape(center,windowSize,windowSize)) )
      % first K patches:
      for k=1:K
        subplot(clusters_at_once, samples_per_cluster+1, (j-1)*(samples_per_cluster+1)+1+k)
        imshow( uint8(reshape(patches(cluster(k),:),windowSize,windowSize)) )
      end
    end
  end
